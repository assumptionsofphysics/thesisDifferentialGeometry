...
NOTE: This may not be necessary, creates problems in intuition with polar coordinates
\begin{defn}
...
\begin{defn}
	A \textbf{vector} is an infinitesimal segment along a line. A \textbf{tangent plane}, then, is a collection of vectors that share a fixed point. 
\end{defn}

NOTE: Need a formal definition of a covector as a linear function of a vector

NOTE: dx is an element of the tangent space such that if you take dx at all points and sum it, you get the original


NOTE: Not integrating, just summing the dx segments
\begin{defn}
...
NOTE: Talk about covectors using the variable omega, and omega will go from V to R

TODO: Put a 1 in there for 1-vectors and 1-forms. Generalizes easier. 


TODO: Try to break this definition, since it doesn't discuss orientation at all. Orientation should be absolutely necessary. 
\begin{prop}
	Every linear 1-functional has a corresponding covector, such that for $f : S \to \mathbb{R}$, $f = \int_{\lambda} f(d\lambda)$. In words, a linear functional applied over a line is the same as an integral of a covector over the infinitesimal segments of that line. Further, for $g : S^{2} \to \mathbb{R}$, $g(\lambda) = \int_{\sigma} g(d\sigma) = f(\partial\sigma) = \int_{\partial\sigma}f(d\partial\sigma)$. 
\end{prop}
...
NOTE: It's a parallelopiped in the k-surface
\begin{defn}
...
TODO: Talk about fixing a corner of the paralellopiped just a point was fixed. A k-form is being given a paralellopiped 

TODO: In wedge, talk about how switching sides we switch the orientation

TODO: After defining the set of all forms, say that every linear k-functional has a corresponding k-form such that... just as in the previous proposition in vectors and covectors 

\begin{defn}
	$V^k$ is the set of all vectors of dimension k and $V = \cup_{k=0}^n V^k$ is the set of all k-vectors. 
	\end{defn}



NOTE: wedge can be discussed as constructing parallelopiped 
\begin{defn}
...
NOTE: talk about vectors and k-vectors using variable v. 
...
TODO: We have to define the exterior derivative. Want to show that the $\eth$ becomes the exterior derivative over the corresponding k-form. We're defining the exterior derivative as the thing that gives us stoke's theorem. 

Would be cool to have a diagram showing the relationship between the exterior derivative, k-functionals, k-forms. So $\eth$ allows us to go from $F_k$ to $F_{k+1}$, exterior derivative goes between $\omega_k$ to $\omega_{k+1}$ and integration goes between $F_k$ and $\omega_k$. 


\begin{defn}
	\textbf{Stoke's Theorem}: For k-form $\omega_k$ and k-surface $\sigma^k$, $\int_{\partial \sigma^k}\omega_k = \int_{\sigma^k}\eth\omega_k$. 
\end{defn}
