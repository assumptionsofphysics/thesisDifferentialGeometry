...
\begin{defn}
	Given k-surface $s^k \in S^k$, if for some point $x \in s^k$ you can construct a open neighborhood around x completely within $s^k$, then x is an \textbf{interior point} of $s^k$. 
\end{defn}

\begin{defn}
	Given k-surface $s^k \in S^k$, if some point $x \in s^k$ is not an interior point, then $x$ is a \textbf{boundary point}.
\end{defn}

\begin{defn}
	Given k-surface $s^k \in S^k$, $\partial s^k \in S^{k-1}$ is the set of all boundary points of $s^k$, or, the \textbf{boundary} of $s^k$. 
\end{defn}

Dimension is one less since at each section of the boundary, one of the dimensions must be held fixed.

$\partial\partial s^k$ is empty since all points on the boundary are interior with respect to the boundary.


\begin{coro}
	Given $s^k \in S^k$, $\partial s^k \in S^{k-1}$, and $\partial\partial s^k = \emptyset$. 
\end{coro}
...
\begin{defn}
	A \textbf{linear function of k-surfaces}, or \textbf{k-functional}, is a function $f_k : S^k \to \mathbb{R}$ such that for $s^k$, $t^k \in S^k$, if $s^k \cap t^k \subseteq \partial s^k \cup \partial t^k$, then $f_k(s^k\cup t^k) = f_k(s^k) + f_k(t^k)$. 
\end{defn}
...
\begin{defn}
	$\Delta : F_k \to F_{k+1}$ such that $\Delta f_k(s^{k+1}) = f_k(\partial s^{k+1})$ is the \textbf{(border, edge, boundary, something like this?) derivative on k-functionals}. 
\end{defn}
...
\begin{prop}
	Let $f_k \in F_k$ be a k-functional, then $\partial\partial f_k = 0 $.
\end{prop}
\begin{proof}
\emph{Something seems off: check with Gabriele that the relationships between boundary operators and boundary derivatives are ok. Just make sure that with def 1.16 it's consistent}

	Let $f_k : S^k \to \mathbb{R}$, $g_{k+1} : S^{k+1} \to \mathbb{R},$ and $h_{k+2}: S^{k+2} \to \mathbb{R}$ be linear functions, and let $s^k$ be an element of $S^k$. $f_k(s^k) = g_{k+1}(\partial s^k) = h_{k+2}(\partial\partial s^k) = f_k(\emptyset) = 0$. 
	
	So, $h^{k+2}(s) = 0$ $\forall$ $s$, and $h^{k+2}$ is the zero function. 
\end{proof}
...
- For k-surface $s^k \in S^k$ and k-functional $f_k: S^k \to \mathbb{R}$, $f_k(s^k) = \int_{s^k}f(ds^k)$. 
...
\begin{defn}
	For k-surface $s^k \in S^k$, $d : S^k \to V^k$ will give the tangent plane of $s^k$. 
\end{defn}
...
- We want to be able to give a line, surface, body, etc. an orientation. This gives us k-vectors. Talk also about their relationship to k-forms. 
...
- Here we will be talking about outer products, and how they behave with k-vectors of different degrees. Introduce also the geometric product as the sum of the symmetric and antisymmetric products

\begin{defn}
	
	The \textbf{outer product} $\wedge : \Sigma^k\times \Sigma^j \to \Sigma^{k+j}$ combines k-vectors antisymmetrically. 
\end{defn}
...
\begin{defn}
	A linear function may be defined \textbf{infinitesimally}, such that $f_k : S^k \to \mathbb{R} = \int_{\sigma^k} \phi_k(d\sigma^k)$. Further, for $g : S^{k+1} \to \mathbb{R}$, $g(\sigma) = \int_{\sigma^k} \gamma(d\sigma^k) = F(\partial\sigma^k) = \int_{\partial\sigma^k}\phi(d\partial\sigma^k)$. 
\end{defn}
...
\begin{defn}
	A \textbf{k-form} $\phi_k : V^k \to \mathbb{R}$ is a skew-symmetric covariant linear functional. 
\end{defn}

\emph{K-forms are functions along k-vectors}

\begin{defn}
	$\Phi_k$ is the set of all forms of dimension k. 
\end{defn}

\begin{defn}
	$\Phi = \cup_{k=0}^n\Phi_k$ is the set of all forms. 
\end{defn}

\begin{defn}
	\textbf{Stoke's Theorem}: For k-form $\phi_k$ and k-surface $\sigma^k$, $\int_{\partial \sigma^k}\phi_k = \int_{\sigma^k}\Delta\phi_k$. 
\end{defn}
