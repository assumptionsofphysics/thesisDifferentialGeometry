...
\begin{defn}
	A \textbf{covector} $\omega_1 : V^1 \to \mathbb{R}$ is a linear function along vectors. 
\end{defn}
...
\begin{prop}
	Every linear 1-functional has a corresponding covector, such that for $f : S \to \mathbb{R}$, $f = \int_{\lambda} \omega_1(d\lambda)$. In words, a linear functional applied over a line is the same as an integral of a covector over the infinitesimal segments of that line. Further, for $g : S^{2} \to \mathbb{R}$, $g(\sigma) = \int_{\sigma} \omega_2(d\sigma) = f(\partial\sigma) = \int_{\partial\sigma}\omega_1(d\partial\sigma)$. 
\end{prop}
...
NOTE: wedge can be discussed as constructing parallelepiped 
\begin{defn}
...
\begin{prop}
	Every linear k-functional has a corresponding k-form, such that for $f_k : S^k \to \mathbb{R}$, $f_k = \int_{\sigma^k} \omega_k(d\sigma^k)$. In words, a linear functional applied over a k-surface is the same as an integral of a k-form over the infinitesimal parallelepipedes of that k-surface. Further, for $g_{k+1} : S^{k+1} \to \mathbb{R}$, $g_{k+1}(\sigma^{k+1}) = \int_{\sigma^{k+1}} \omega_{k+1}(d\sigma^{k+1}) = f_k(\partial\sigma^{k+1}) = \int_{\partial\sigma^{k+1}}\omega_k(d\partial\sigma^{k+1})$. 
\end{prop}
...
\begin{defn}
	$\eth : \Omega_k \to \Omega_{k+1}$ is the \textbf{exterior derivative} on k-forms. 
\end{defn}

\begin{prop}
	The interior operator $\eth$ on k-functionals is equivalent to the exterior derivative on k-forms. 
\end{prop}

\begin{proof}
	
	Consider k-surface $\sigma$ and (k-1)-functional $f$ with associated (k-1)-form $\omega$. 
