...
\begin{defn}
	Given $\eth : F_k \to F_{k+1}$ and $f_k \in F_k$, \textbf{boundary functional} $\eth f_k \in F_{k+1}$ is the functional with the property that $\eth f_k(\sigma^{k+1}) = f_k(\partial \sigma^{k+1})$. 
\end{defn}
