...
[\textbf{Start a new section discussing this stuff, and then talk about perpendicularity usw after the definition of wedge product to differentiate it from the cross product. Basically want to have two point: 1: The cross product needs perpendicularity, which we don't have; 2: even if we had it, it would depend on the dimensionality of the manifold, so we don't want it anyways. We can talk about pseudovectors afterwards (they're 2-forms in 3D). We can then talk about how in SR (4D), the perpendicular (hodge dual) is still a tensor rank 2.}]
...
A k-form may operate on multiple vectors if the sum of the ranks of the vectors equal k. That is, for $v^i \in V^i$ and $v^j \in V^j$, such that $i + j = k$, we write $\omega_k(v^i, v^j)$ to say that $\omega_k$ has been passed two vectors, from which we will get a single scalar. [\textbf{This comes out of nowhere. Why does this happen?}]
