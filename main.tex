...
Let us now impose the condition that $\lambda$ is closed, meaning that $\partial\lambda = 0$, while continuing to consider the work done by both drag and some conservative force. Effectively, this means that its beginning and ending points are the same. We will in general have that $W(\lambda)$ takes on some value which depends on the path taken between the beginning and ending points of $\lambda$. [\textbf{$W(\partial\lambda)$ doesn't work}] However, $W(\partial\lambda) = W(\emptyset) = 0$, meaning that if we are measuring work only on the boundary of the line, in this case, we would find that no work had been done.
