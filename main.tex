...
To give an example where this condition is not fulfilled, suppose we have a line $\lambda$, and we want to determine its length. A sequence of lines $\{\lambda_i\}^{\infty}_{i=1}$ converges to $\lambda$, but the length of the converging sequence does not. This is because length varies nonlinearly with horizontal and vertical components. In other words, the length of a straight line $\lambda^*$ is $\sqrt{x^2 + y^2}$, where $x$ and $y$ are its horizontal and vertical components. So, the length of the limit is not the limit of the length, and a length function is not a linear functional. 
