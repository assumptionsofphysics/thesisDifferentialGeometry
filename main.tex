...
\textbf{Motivate skew-symmetric using the flipping axis example, put it after the wedge product saying that it's a result of the properties}
...
\textbf{Through the wedge product, we can define the form based on the sides, rather than on the whole k-vector. The purpose hre would be to describe the forms in standard way. A form is usually something that takes n-vectors to a scalar, so we want the wedge product to define that. "In light of hte wedge product, we can define forms on a list of 1-vectors that will have these wedge product properties"}
A k-form may operate on multiple vectors if the sum of the ranks of the vectors equal k. That is, for $v^i \in V^i$ and $v^j \in V^j$, such that $i + j = k$, we write $\omega_k(v^i, v^j)$ to say that $\omega_k$ has been passed two vectors, from which we will get a single scalar. We may equivalently express $\omega_k(v^i, v^j)$ as an operation on the wedge of $v^i$ and $u^j$ such that \begin{gather}\omega_k(v^i, u^j) = \omega_k(v^i \wedge u^j) = -\omega_k(u^j \wedge v^i).\end{gather} We can say also that \begin{gather}\omega_k(v^i \wedge u^j) = \omega_i(v^i)\wedge \omega_j(u^j) = -\omega_j(u^j) \wedge \omega_i(v^i).\end{gather} We see therefore that the skew-symmetric nature of k-vectors carries over to the k-forms. 
\textbf{in 2.15, given two i and j forms, we can always think of creating a new i+j form}


\textbf{k-vectors can be decomposed, but not k-forms, since k-vectors are defined on the point with vectors. Forms change for every point on the space, while vectors in the vector space is defined locally within a single tangent space. Similar to marginal and joint distributions}
