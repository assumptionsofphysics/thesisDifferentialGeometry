...
Within differential forms lies the generalization of the mathematical structure underlying potentials, forces, and fields. Consider a 

There are two classes of k-forms here we want to focus on: \textbf{closed} forms and \textbf{exact} forms.
 
\begin{defn} 
	A k-form \textbf{$\omega$} is closed if $\eth\omega = 0$. 
\end{defn}

\begin{defn} 
	A k-form \textbf{$\omega$} is exact if there exists a (k-1)-form $\alpha$ such that $\omega = \eth\alpha$.
\end{defn}

It follows from this that all exact forms are closed (since $\eth\eth\omega = 0$ for all forms), but not all closed forms are exact. Consider for instance the field $F = y e_x + x e_y$. $F$ is closed, since $\frac{\partial F_x}{\partial y} - \frac{\partial F_y}{\partial x} = 0$, but it is not the exterior derivative of any other form, so it is not exact. A form being exact relates to a force being conservative. The field $F$, therefore, does not represent a conservative force field (it is not path independent). However, a region may be constructed such that the field is \emph{locally} conservative. The only requirement to this region is that it does not include the origin. Thus, a field may be locally conservative, even if it is not globally conservative. Therefore, a potential exists in that local region. 
