...
\emph{It is confusing as it is an abuse of terminology. The position coordinate system is the real idea, it's not that coordinate systems are more general, it's that we're using the same word for more things that are sort of similar}
...
\emph{flows well with "object." Saying "object" is probably fine}
...
\emph{Let the geography example flow into this paragraph. Otherwise this paragraph seems to be out of nowhere}
...
\begin{defn}
	A \textbf{manifold} is a set of physical objects $X$ such that for any $x \in X$ there exists a $U \subset X$ that contains $x$ and upon which a coordinate system $Q$ is defined. \textbf{Include in here dimension of a manifold. Use "dimension," not "degrees of freedom"}
\end{defn}


Earlier, weather was given as an example of a set of possible cases of a system, with readings such as temperature and humidity allowing us to define a coordinate system. In this context, the "manifold" would be the set of possible weather configurations, with each point being a single weather report. 


\emph{This paragraph can go before with the definition of manifold as the motivating example}
...
\emph{Say that some of these manifolds are not geometrical, in the sense that there are no distances. What does it mean to have the distance between weather reports?}
...
\textbf{\emph{Give a motivating example here}}
...
A good example of a k-functional, and indeed, the one that will be carried through to the next section, is a work function. Given a path, the work function will return the force applied on an object between a given starting point and ending point on that path.
