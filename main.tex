...
\begin{defn}
	Suppose we have a k-functional $f \in F_k$ and k-surface $\sigma \in S^k$. A \textbf{gauge transformation} $G: F_k \to F_k$ is a transformation on functionals such that if $\sigma$ is closed, then $G(f)(\sigma) = f(\sigma)$, while if $\sigma$ is not closed, then $G(f)(\sigma) \neq f(\sigma)$. 
\end{defn}
...
\begin{defn}
	Let $\sigma$ be a (k+1)-surface, let $f$ be a k-functional with associated k-form $\omega$, and let $g$ be a (k+1)-functional with associated (k+1)-form $\gamma$. We define the \textbf{exterior derivative}\footnote{In most texts on the subject (cf. Curtis and Miller, "Differential Manifolds and Theoretical Physics") $\eth : \Omega_k \to \Omega_{k+1}$ , the exterior derivative is denoted simply with a $d$} as the tool that allows $\omega$ to act on the boundary of $\sigma$, such that if
	
	\begin{gather}f(\partial\sigma) = \int_{\partial\sigma}\omega(d\partial\sigma)\end{gather} and \begin{gather}  g(\sigma) = \int_\sigma \gamma(d\sigma), \end{gather}
	then if
	\begin{gather} g(\sigma) = f(\partial\sigma) = \eth f(\sigma),\end{gather} 
	then
	\begin{gather} \int_\sigma \gamma(d\sigma) = \int_{\sigma}\eth\omega(d\sigma) \end{gather}
\end{defn}
