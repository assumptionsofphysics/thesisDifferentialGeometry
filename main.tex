...
\textit{Point out that we're generalizing the concept of a coordinate system, so that position is the template for the general case}
...
\textit{stops at the edge. Boundary could be on the submanifold or not, if not it would be a limit point. May not be necessary to talk about, so that people won't get confused}
