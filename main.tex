...
\end{tikzcd}
\end{center}
\caption{This is commutative diagram shows how sets relate to each other, and what functions are used to relate them. The double sided arrows denote a one-to-one relationship between the elements of the two sets related using the operation above the arrow}
\end{figure}
...
This lack of distance is a result of the fact that we are not speaking geometrically. That is, we do not have geometry defined (neither Riemannian, nor symplectic). Therefore, not only do we not have a notion of distance, we also do not have a notion of angles. To demonstrate the implications of this lack of distance and angles, consider the rotation of a coordinate system. First of all, say we have an x-y coordinate system, describing position. We can rotate this system freely. If we rotate it by 90$^o$, we would be holding invariant the angles and the distances between the points under rotation. 

\textbf{Somewhat abstract right now, and I don't have a solid intuitive understanding yet. Play with this later and write a better explanation}
\textbf{Talk about rotation of the coordinate system. It works in x-y, but not in p-V. Allows us to talk about that not all spaces are geometrical. So, we don't have a notion of geometry here.}
