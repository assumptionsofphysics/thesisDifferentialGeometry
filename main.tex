...
\textbf{Have to say what the $e_i$ and $e^j$ actually are. $e_i$ isn't really a vector}
\begin{defn}
	The \textbf{cobasis} of $\Omega_1$ is a set of linearly independent covectors such that any element in $\Omega_1$ may be constructed using the basis covectors. For a set of forms with underlying field $\mathbb{R}^3$, the cobasis may be written as $(e^1, e^2, e^3)$. When a vector is given to a basis covector, we get the component of the vector along that direction. A basis vector $e_i$ and basis covector $e^j$ relate to each other as $e_ie^j = \delta^j_i$\footnote{$\delta^i_j$ is the Kronecker delta, defined to equal 1 when i = j, and 0 otherwise}. \textbf{It has to be defined on the coordinate system. Only thing that really needs to be said is that a basis covector acts on a vector to return a component along that direction. $e^i dP = dx^i$}
\end{defn}
...
\textbf{Put the very last piece as the second term after the omega by itself, saying that omega is the same in both expressions. We don't konw yet what omega' is, but we can do these things with the chain rule. Then we put the dx'j / dp as e'j}
