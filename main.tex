...
In any introductory class on classical mechanics, you will run into the words "conservative" and "nonconservative" forces. If a force is conservative, the work along a path under the influence of that force depends only on the starting and ending points of that path. In a class on classical mechanics, you would then be told that such a force $f$ is simply a gradient of a potential $V$; i.e. $f = -\nabla V$. Instead of that, we will be saying that in such a case, only the \textbf{boundary} of the path matters; or in terms of the force, that such a force is a \textbf{boundary functional}. 
...
It is nevertheless interesting to talk about objects that undergo such sign flips under reflection. Consider an x-y coordinate plane, with x in the horizontal direction and y in the vertical direction. The coordinate system may be rotated freely (creating an equivalence class of directions), but try swapping x and y, so that x is now vertical and y is now horizontal. The original coordinate system can no longer be reached simply through rotation; it would require a flipping of one of the axes. This allows us to define the \textbf{wedge product}: a tool that joins k-vectors in such an antisymmetric way.
